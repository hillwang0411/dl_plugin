...
#include "tensorflow_plugin/src/stream_executor/cuda/cuda_driver.h"
#include "tensorflow_plugin/src/stream_executor/cuda/cuda_gpu_executor.h"
#include "tensorflow_plugin/src/stream_executor/cuda/cuda_platform_id.h"
...
//      port::error::INTERNAL,
//      absl::StrFormat(
//          "not yet implemented %s", __func__));
//}
...
port::StatusOr<GpuExecutor*> CudaPlatform::ExecutorForDevice(int ordinal) {
  StreamExecutorConfig config;
...
port::StatusOr<GpuExecutor*> CudaPlatform::GetExecutor(
    const StreamExecutorConfig& config) {
  auto executor = new GpuExecutor(config.plugin_config);
  auto init_status = executor->Init(config.ordinal, config.device_options);
  if (!init_status.ok()) {
    return port::Status(
        port::error::INTERNAL,
        absl::StrFormat(
            "failed initializing GpuExecutor for CUDA device ordinal %d: %s",
